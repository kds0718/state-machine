...
contract('StateMachine', accounts => {
  ...
  it('should allow you to add transitions after steps and before marked immutable', async () => {
    let currentState;
    currentState = await stateMachine.currentStateId.call();
    assert.equal(web3.toUtf8(currentState), state0);
    ...
    await stateMachine.setDummyCondition(state1, state2);

    await stateMachine.allowFunctionHelper(state2, dummyFunctionSelector); 
    await stateMachine.setDummyTransitionEffect(state0, state2); 
    await stateMachine.createTransitionHelper(state1, 'state4');
  ...
  it('should not allow you to add transitions after being marked immutable', async() => {
    let currentState;
    currentState = await stateMachine.currentStateId.call();
    assert.equal(web3.toUtf8(currentState), state0);
    ...
    await stateMachine.makeImmutableHelper(); 

    await expectThrow(stateMachine.setDummyCondition(state1, state2));
    await expectThrow(stateMachine.setDummyTransitionEffect(state0, state2));
    await expectThrow(stateMachine.createTransitionHelper(state1, 'state4'));
